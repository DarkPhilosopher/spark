...
| **💬** | Chat — the exact same conversation the browser editor's own box has, one per hosted game; anything typed without a leading `/` is said to the others, which only means anything in `LIVE` mode — there's nobody else to talk to in a local, single-tab copy, but whatever you type is kept in this log regardless, with a note if it wasn't actually sent to anyone. **Always open** — on from the moment a game loads, with no way to close it; tapping `💬` just (re-)focuses the input. Sending a message with Enter never closes it, of course. Its own panel fills the whole screen edge to edge except the top bar and the movement pad, both still fully visible and usable above and below it — no more small floating card, since it can't be dismissed to get it out of the way any more. `/who` and `/clear` work either way. `/help` shows the game's own "how to play" first, if it has one (`games/outpost.json` does), then the full command reference grouped by type; `/help commands` (add `description` too) shows just that reference on its own. `/mine`/`/attack`/`/recruit`/`/dismiss x y` act on an exact spot the same as touching it would — gather ore, hit a bandit, recruit or release a bare/led companion/worker/soldier — one square away at most; `/units` lists everything you own or possess with its location, `/list` widens that to every entity in the world whether it's yours or not (kind, position, health, leader, inventory), `/name x y <new name>` renames whatever's yours there. All local-play only like Build/Harvest above. Nothing here is ever silently lost — the whole log is kept, titled into numbered pages of 10; `/log n` scrolls to one, `/forget n` removes one for good, its own number never reused even after |
